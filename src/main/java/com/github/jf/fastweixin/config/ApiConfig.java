package com.github.jf.fastweixin.config;

import com.github.jf.fastweixin.response.GetJsApiTicketResponse;
import com.github.jf.fastweixin.response.GetTokenResponse;
import com.github.jf.fastweixin.exception.WeixinException;
import com.github.jf.fastweixin.handle.ApiConfigChangeHandle;
...
public final class ApiConfig extends Observable implements Serializable {
     ...
     * @param appid       公众号appid
     * @param secret      公众号secret
     * @param enableJsApi 是否启动js weixin
     */
    ...
    private void initJSToken(final long refreshTime) {
        ...
        String url = "https://weixin.weixin.qq.com/cgi-bin/ticket/getticket?access_token=" + accessToken + "&type=jsapi";
        NetWorkCenter.get(url, null, new NetWorkCenter.ResponseCallback() {
            @Override
            public void onResponse(int resultCode, String resultJson) {
                if (HttpStatus.SC_OK == resultCode) {
